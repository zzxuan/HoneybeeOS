	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				io_sti();
				if(mouse_decode(&mdec,i)==1)//Ú¾Š®‘l?”˜
				{
...
void wait_KBC_sendready(void)//“™‘Ò??T§?˜Hy?Š®?
{
	while(1)
	{
...
void enable_mouse(struct MOUSE_DEC *mdec)//ŒƒŠˆ‘l?
{
	wait_KBC_sendready();
