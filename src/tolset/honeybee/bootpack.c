...
void HariMain(void)
{
	...
	make_window8(buf_win,160,52,"HoneybeeOS");
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* 键盘数据 */
				...
				if (i < 0x54 + 256) {
					if (keytable[i - 256] != 0 && cursor_x < 144) { //一般字符显示
...
void task_b_main(struct SHEET *sht_back)
{
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			io_sti();
			if (i == 1) { /*  */
				sprintf(s,"taskb count = %11d",count);
				putfonts8_asc_sht(sht_back,0,144,COL8_FFFFFF,COL8_008484,s,127);
				timer_settime(timer_put, 1);
			} else if (i == 100) {
				sprintf(s, "taskb count = %11d", count - count0);
				putfonts8_asc_sht(sht_back, 0, 128, COL8_FFFFFF, COL8_008484, s, 127);
				count0 = count;
				timer_settime(timer_1s, 100);
			}
		}
